...
public: static size_t totalMemory; // статическая переменная для хранения общего количества памяти
...
public: Rectangle(int id, double width, double height): id(id),
                                                        width(width),
                                                        height(height) {
    ...
    bool equals(const Object & other) const override {
        if (const Rectangle * otherRect = dynamic_cast <
...
public: IsoscelesTrapezoid(int id, double upperBase, double lowerBase, double height): id(id),
                                                                                       upperBase(upperBase),
                                                                                       lowerBase(lowerBase),
                                                                                       height(height) {
        GeometricObject::totalMemory += sizeof( * this); // увеличение общей памяти на размер объекта
    ...
    bool equals(const Object & other) const override {
        if (const IsoscelesTrapezoid * otherTrap = dynamic_cast <
                const IsoscelesTrapezoid * > ( & other)) {
...
int main() {
    ...
    for (const auto &shape: shapes) {
        std::cout << shape->getName() << " (id=" << shape->getId() << ", area=" << shape->getArea() << ", perimeter="
